...
module.exports = function(locals) {
    ...
    IndexAnything.prototype.push = function(post) {
      ...
      if (!_.has(post, ia.index)) {
        return
      }
      // deal with indexes containing multiple values (like tags)
      let values = _.flatten([post[ia.index]])
      _.each(values, function(value) {
        value = S(value).slugify().s
        ia.posts[value] = ia.posts[value] || []
        ia.posts[value].push(post)
      })
    }
    ...
    IndexAnything.prototype.getList = function() {
      ...
      return _.map(_.keys(ia.posts), function(key) {
        return {
          key: key,
          title: S(key).humanize().titleCase().s,
          date: moment(),
          path: ia.getPath(key),
          count: _.size(ia.posts[key])
        }
      })
    }
    ...
    IndexAnything.prototype.getPages = function() {
      ...
      ia.posts.index = ia.getList()
      return _.map(ia.posts, function(posts, key) {
        // add handy methods for templates
        posts = _(posts)
        let indexString = S(ia.index).humanize().titleCase().s
